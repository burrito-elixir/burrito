...
use std::io::{prelude::Read, prelude::Write, Cursor};
...
fn main() {
    ...
    if needs_install {
        let result: Result<(), anyhow::Error> =
            decompress_payload(&base_install_dir);
        if result.is_err() {
...
fn decompress_payload(destination_path: &Path) -> Result<()> {
    ...
    let mut decompressor: XzDecoder<&[u8]> = XzDecoder::new(&payload[..]);
    let mut decompressed_data: Vec<u8> = Vec::new();
    let decompressed_size: usize = decompressor.read_to_end(&mut decompressed_data)?;
