defmodule Burrito.Util.ERTSUrlFetcher do
  ...
  def fetch_version(os, libc, otp_version) when is_binary(otp_version) and is_atom(os) and is_atom(libc) do
    all_versions = fetch_all_versions()

    res = if os == :windows do
      all_versions.windows
    else
      all_versions.posix
    end

    platform_string = if os == :windows do
      "win64"
    else
      if os == :darwin do
        "darwin"
  ...
  defp get_gh_pages(url, page \\ 1, acc \\ []) do
    ...
    case Req.get!(url <> "&page=#{page}") do
      %Req.Response{status: 200, body: []} -> acc
